...
QUOTA_GRANTS = [
    ...
    {"zone": "us-central1-a", "type": "v5e", "chips": 64, "mode": "spot", "usable": False, "internet": False},
    {"zone": "europe-west4-b", "type": "v5e", "chips": 64, "mode": "spot", "usable": False, "internet": False},
]

# Granted zones to scan (usable only)
GRANTED_ZONES = ["europe-west4-a", "us-east1-d", "us-central2-b"]
...
def query_vm_live(vm_cfg, exp_name):
    ...
    if not heartbeats:
        # No heartbeats — either not started or XLA compiling
        # Check if assignment exists
        assign_raw = run_cmd(f"{GCLOUD} storage cat {prefix}/assignments/{tpu}.json 2>/dev/null", timeout=10)
        if assign_raw:
            return {
                "status": "XLA COMPILE", "color": "yellow",
                "procs": 0, "step": 0, "total_steps": 0,
                "train_loss": "—", "best_val": "—", "time_left": "~10min",
                "done": 0, "config": "—",
            }
        return {"status": "NO ASSIGNMENT", "color": "dim"}

    # Find freshest heartbeat
    now = time.time()
    best_hb = max(heartbeats, key=lambda h: h.get("timestamp", 0))
    ...
    # Determine status from heartbeat age
    if age_s > 1500:  # 25 min = dead
        status, color = "DEAD", "red"
    elif age_s > 600:  # 10 min = stale
...
def build_experiment_panel(exp_name, exp_cfg, validated, total, state, vm_configs=None):
    ...
    if assignments:
        ...
        for vm, labels in sorted(assignments.items()):
...
def build_dashboard(exp_name=None):
    ...
    # Experiment
    if exp_name:
        output_parts.append(build_experiment_panel(exp_name, exp_cfg, validated, total, state, vm_configs))
...
def main():
    ...
    parser.add_argument("--exp", default=None, help="Experiment name (e.g., exp12_1)")
    ...
    exp_name = args.exp or os.environ.get("EXP")

    console = Console()

    if args.once:
        parts = build_dashboard(exp_name)
        for part in parts:
            console.print(part)
        return
