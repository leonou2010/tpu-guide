...
if [ ! -f "$MODEL_PATH/config.json" ]; then
    ...
    gsutil -m cp -r "gs://gcp-researchcredits-blocklab-europe-west4/models/SmolLM2-135M/*" \
...
# exp13 — ALWAYS refresh to get latest train_v2_tpu.py
echo "Refreshing exp13 code..."
gcloud storage cp "${BUCKET}/code/sf_bema_code_exp13.tar.gz" \
    /tmp/sf_bema_code_exp13.tar.gz 2>/dev/null || \
gcloud storage cp "gs://gcp-researchcredits-blocklab-europe-west4/code/sf_bema_code_exp13.tar.gz" \
    /tmp/sf_bema_code_exp13.tar.gz 2>/dev/null || \
gsutil cp "gs://gcp-researchcredits-blocklab-1-us-central2/code/sf_bema_code_exp13.tar.gz" \
    /tmp/sf_bema_code_exp13.tar.gz 2>/dev/null || true
[ -f /tmp/sf_bema_code_exp13.tar.gz ] && \
    tar xzf /tmp/sf_bema_code_exp13.tar.gz -C ~/sf_bema/experiments/ 2>/dev/null || true

# exp12_1 — only if missing
if [ ! -d ~/sf_bema/experiments/exp10_smollm2_smoltalk/exp12c_tpu ]; then
    echo "Downloading exp12_1 code..."
    gcloud storage cp "gs://gcp-researchcredits-blocklab-europe-west4/code/sf_bema_code_exp12_1.tar.gz" \
        /tmp/sf_bema_code_exp12_1.tar.gz 2>/dev/null || \
    gsutil cp "gs://gcp-researchcredits-blocklab-1-us-central2/code/sf_bema_code_exp12_1.tar.gz" \
        /tmp/sf_bema_code_exp12_1.tar.gz 2>/dev/null || true
    [ -f /tmp/sf_bema_code_exp12_1.tar.gz ] && \
        tar xzf /tmp/sf_bema_code_exp12_1.tar.gz -C ~/sf_bema/experiments/ 2>/dev/null || true
fi
...
if [ ! -d "${DATA_DST}/train" ]; then
    ...
    gcloud storage cp -r "${BUCKET}/data/smoltalk/data/val" \
        "$DATA_DST/" 2>/dev/null || \
    gsutil -m cp -r "gs://gcp-researchcredits-blocklab-europe-west4/data/smoltalk/data/train" \
        "$DATA_DST/" 2>/dev/null && \
    gsutil -m cp -r "gs://gcp-researchcredits-blocklab-europe-west4/data/smoltalk/data/val" \
        "$DATA_DST/" 2>/dev/null || true
    echo "Data: $(ls $DATA_DST 2>/dev/null | wc -l) dirs"
fi

# Symlink data for exp13
if [ -d ~/sf_bema/experiments/exp10_smollm2_smoltalk/data ] && \
   [ ! -e ~/sf_bema/experiments/exp13_smollm2_smoltalk/data ]; then
    ln -sf ~/sf_bema/experiments/exp10_smollm2_smoltalk/data \
        ~/sf_bema/experiments/exp13_smollm2_smoltalk/data
fi
...
export GCS_CHECKPOINT_DIR=${BUCKET}/checkpoints/exp13
