...
# Zone configs: zone accelerator_type version max_vms name_prefix
ZONES=(
  "europe-west4-a v6e-8 v2-alpha-tpuv6e 8 v6e-ew4a"
  "us-east1-d v6e-8 v2-alpha-tpuv6e 8 v6e-ue1d"
  "us-central2-b v4-8 tpu-ubuntu2204-base 8 v4-uc2b"
  "europe-west4-b v5litepod-4 v2-alpha-tpuv5-lite 16 v5e-ew4b"
...
while true; do
  ...
  for zoneconf in "${ZONES[@]}"; do
    read -r zone accel version max_vms prefix <<< "$zoneconf"
    ...
    while IFS=',' read -r name state; do
      ...
      case "$state" in
        PREEMPTED)
          ...
          $GCLOUD alpha compute tpus tpu-vm delete $name --zone=$zone --project=$PROJECT --quiet 2>&1 | tail -1
          cleanup_vm_gcs "$name"
          clear_creating "$name"
          ;;
        READY)
    ...
    if [ "$total" -lt "$max_vms" ] && ! $HC_BLOCKED; then
      created=0
