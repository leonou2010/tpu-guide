...
def pull_and_validate(exp_name, results_base):
    ...
    for path in completed_paths:
        ...
        if valid:
            ...
            print(f"  [validate] {label}: INVALID ({reason}) — moving to invalidated/")
            os.remove(local_path)
            # Move completion record to invalidated/ so it doesn't re-validate.
            # populate.py will re-add to pending/ on next reconcile if the task is still needed.
            invalidated_path = f"{CONTROL_PLANE}/invalidated/{task_id}.json"
            if not gcs_move(path, invalidated_path):
                gcs_delete(path)
                print(f"  [validate] {label}: WARNING — gcs_move failed, deleted completion record")
...
def main():
    ...
    while True:
        ...
        if all_done:
            ...
            print(f"Run: bash ~/distributed_tpu_training/cleanup_gcs.sh --dry-run && bash ~/distributed_tpu_training/cleanup_gcs.sh")
            print(f"{'='*60}\n")
            break
