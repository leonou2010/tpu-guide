...
def gcs_write(path, content):
    try:
        if _use_gsutil():
            proc = subprocess.run(
                ['bash', '-c', f'cat <<\'GCSEOF\' | gsutil cp - {path}\n{content}\nGCSEOF'],
...
def claim_task(worker_id):
    ...
    random.shuffle(pending)  # avoid thundering herd

    for task_path in pending:
        task_id = os.path.basename(task_path).replace('.json', '')

        # Skip if already claimed
        if gcs_exists(f"{CONTROL_PLANE}/running/{task_id}.json"):
            continue
        ...
        # Write claim to running/
        claim = {**task, 'worker_id': worker_id, 'claimed_at': time.time()}
        gcs_write(f"{CONTROL_PLANE}/running/{task_id}.json", json.dumps(claim))

        # Delete from pending
        gcs_delete(task_path)

        # Verify we won the claim
        verify = gcs_read(f"{CONTROL_PLANE}/running/{task_id}.json")
        if verify:
            try:
                v = json.loads(verify)
...
def complete_task(task_id, worker_id, result_summary=None):
    """Mark task as completed. Move from running/ to completed/."""
    completion = {
        'task_id': task_id,
        'worker_id': worker_id,
    ...
    gcs_write(f"{CONTROL_PLANE}/completed/{task_id}.json", json.dumps(completion))
...
def fail_task(task_id, worker_id, error_msg, retry=True, max_retries=10):
    """Handle failed task. Move back to pending/ (retry) or to failed/."""
    ...
    if retry and retries <= max_retries:
        ...
        gcs_write(f"{CONTROL_PLANE}/pending/{task_id}.json", json.dumps(task))
    else:
        # Permanently failed
        task['retries'] = retries
        task['last_error'] = error_msg
        task['failed_at'] = time.time()
        gcs_write(f"{CONTROL_PLANE}/failed/{task_id}.json", json.dumps(task))
...
def reclaim_stale(stale_ttl_s=300, startup_grace_s=2700):
