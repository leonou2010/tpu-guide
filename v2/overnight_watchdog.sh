...
#
# Usage:
#   EXPERIMENTS="exp14:200 exp15:300" nohup bash ~/distributed_tpu_training/v2/overnight_watchdog.sh \
...
ensure_monitor() {
  ...
  if ! pgrep -f 'distributed_tpu_training/v2/monitor.py' >/dev/null 2>&1 || [ "$last_active_age" -gt 600 ]; then
    if [ "$last_active_age" -gt 600 ]; then
      log "WATCHDOG: monitor.py STALLED (last active ${last_active_age}s ago) — killing and restarting"
      pkill -f 'distributed_tpu_training/v2/monitor.py' 2>/dev/null || true
      sleep 2
    else
    ...
    RESULTS_BASE="$RESULTS_BASE" python3 -u ~/distributed_tpu_training/v2/monitor.py \
...
ensure_vm_requester() {
  ...
  bash ~/distributed_tpu_training/v2/vm_requester.sh >> /tmp/vm_requester.log 2>&1 &
...
requeue_failed() {
  ...
  for path in $failed_list; do
    ...
    if [ "${retries:-99}" -lt "$MAX_RETRIES_REQUEUE" ]; then
      # Move back to pending — keep existing retries counter intact so code bugs
      # don't loop forever. Only strip execution metadata (worker/claim/failed_at).
      echo "$raw" | python3 -c "
import sys, json
...
trigger_repair() {
  ...
  for exp in "${EXP_NAMES[@]}"; do
    ...
    EXP=$exp python3 ~/distributed_tpu_training/v2/populate.py 2>&1 | tail -5
  done
...
report_progress() {
  ...
  active_workers=$(python3 ~/distributed_tpu_training/v2/check_progress.py 2>/dev/null | \
