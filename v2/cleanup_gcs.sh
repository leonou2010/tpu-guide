...
# KEEPS: coord/<exp>/results/ — babysitter result uploads (only durable copy until monitor archives locally)
# DELETES: checkpoints (~459 GiB), coord_v2 history (heartbeats/telemetry/logs/completed/failed)
#
...
# ── 3. Old push-coordinator artifacts ────────────────────────────────────────────
# IMPORTANT: Do NOT delete ${b}/coord/ entirely.
# babysitter.py uploads results to ${b}/coord/<exp>/results/ — this is the only
# durable copy until monitor.py has downloaded + validated them locally.
# Deleting coord/ before archiving = data loss.
#
# Safe to delete only after: monitor has validated all tasks (validated/ count == target)
# AND you have confirmed local validated/ is complete.
#
# To delete old push-coordinator state only (no results):
#   for b in ...; do gsutil -m rm -r "${b}/coord/<exp>/done/" "${b}/coord/<exp>/heartbeat/"; done
# To delete results after confirmed local archive:
#   for b in ...; do gsutil -m rm -r "${b}/coord/<exp>/results/"; done
echo "--- coord/ (SKIPPED — contains babysitter results at coord/<exp>/results/) ---"
echo "  Run separately after confirming local validated/ is complete."
