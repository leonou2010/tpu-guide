...
echo "[setup_v6e_eu] TPU_NAME=${TPU_NAME} ZONE=${ZONE} BUCKET=${BUCKET}"
echo "[setup_v6e_eu] EXP_DIR=${EXP_DIR}"
echo "[setup_v6e_eu] GCS_OUT=${GCS_OUT}"

export TPU_NAME ZONE BUCKET CONTROL_PLANE RUNSTAMP TEST_ID GCS_OUT

# Ensure experiment code exists on a fresh VM.
# NOTE: Don't pre-create EXP_DIR before this, or deploy_ew4a.sh will skip downloading code.
if [ ! -f "${EXP_DIR}/exp13_tpu_rerun/train_tpu.py" ]; then
  echo "[setup_v6e_eu] Experiment code missing — downloading sf_bema_exp13_rerun3.tar.gz..."
...
if [ -L "${DATA_DIR}" ]; then
  echo "[setup_v6e_eu] Removing data symlink: ${DATA_DIR} -> $(readlink "${DATA_DIR}")"
  rm -f "${DATA_DIR}"
fi
mkdir -p "${DATA_DIR}"

echo "[setup_v6e_eu] Ensuring dataset present (smoltalk)..."
...
[ -d "${DATA_DIR}/train" ] || { echo "[setup_v6e_eu] ERROR: missing ${DATA_DIR}/train after download"; exit 1; }
[ -d "${DATA_DIR}/val" ] || { echo "[setup_v6e_eu] ERROR: missing ${DATA_DIR}/val after download"; exit 1; }

echo "[setup_v6e_eu] Ensuring model present..."
mkdir -p "${MODEL_DIR}"
if [ ! -f "${MODEL_DIR}/config.json" ]; then
  gsutil -m cp "${BUCKET}/models/SmolLM2-135M/*" "${MODEL_DIR}/" 2>/dev/null || true
fi
[ -f "${MODEL_DIR}/config.json" ] || { echo "[setup_v6e_eu] ERROR: missing ${MODEL_DIR}/config.json after model download"; exit 1; }

echo "[setup_v6e_eu] Running environment deploy (deploy_ew4a.sh)..."
TMP_DEPLOY="/tmp/deploy_ew4a.sh"
gsutil cp "${BUCKET}/pull_code_v3/deploy_ew4a.sh" "${TMP_DEPLOY}" 2>/dev/null || \
  gsutil cp "gs://gcp-researchcredits-blocklab-europe-west4/pull_code_v3/deploy_ew4a.sh" "${TMP_DEPLOY}" 2>/dev/null
chmod +x "${TMP_DEPLOY}"
TPU_NAME="${TPU_NAME}" ZONE="${ZONE}" BUCKET="${BUCKET}" CONTROL_PLANE="${CONTROL_PLANE}" FORCE_REDEPLOY=1 \
  bash "${TMP_DEPLOY}" || true

echo "[setup_v6e_eu] Stopping babysitter to run isolated smoke test..."
pkill -9 -f babysitter.py 2>/dev/null || true
pkill -9 -f train_tpu.py 2>/dev/null || true
pkill -9 -f train_v2_tpu.py 2>/dev/null || true

echo "[setup_v6e_eu] Running smoke training..."
cd "${EXP_DIR}"
